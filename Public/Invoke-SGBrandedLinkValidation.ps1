Function Invoke-SGBrandedLinkValidation {
    [CmdletBinding()]
    param(
    ...
    Process {
        Try {
            $Response = Invoke-WebRequest -Method Post -Uri "https://api.sendgrid.com/v3/whitelabel/links/$($DomainID)/validate" -Headers @{'authorization'="Bearer $($APIKey)";'content-type'='application/json'} -UseBasicParsing
        } Catch {
            $errorcode = $_
            ...
            if((ConvertFrom-Json $Response.Content).valid -ne $true) {
                foreach($ErrProp in ((ConvertFrom-Json $Response.Content).validation_results | Get-Member -MemberType NoteProperty).Name) {
                    if(!(ConvertFrom-Json $Response.Content).validation_results.$ErrProp.valid) {
                        Write-Error -Message (ConvertFrom-Json $Response.Content).validation_results.$ErrProp.reason -Category InvalidResult
                    }
                }
            }
